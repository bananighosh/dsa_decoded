        ...
        for r in range(rows):
            for c in range(cols):
                prefixSum[r][c] = mat[r][c] \
                                    + (prefixSum[r - 1][c] if r > 0 else 0) \
                                    + (prefixSum[r][c - 1] if c > 0 else 0) \
                                    - (prefixSum[r - 1][c - 1] if r > 0 and c > 0 else 0)

        def squareSum(r, c , r2, c2):
            # ref notebook for this calculation and visualization
            currSquareSum = prefixSum[r2][c2] \
                        - (prefixSum[r - 1][c2] if r > 0 else 0) \
                        - (prefixSum[r2][c - 1] if c > 0 else 0) \
                        + (prefixSum[r - 1][c - 1] if r > 0 and c > 0 else 0)
